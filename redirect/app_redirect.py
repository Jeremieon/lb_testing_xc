...
@app.get("/")
async def root():
    return {"app": "redirect-app", "message": "Click /redirect-temp or /redirect-perm to test redirects"}

@app.get("/redirect-temp")
async def redirect_temporary():
    """Returns 302 temporary redirect"""
    return RedirectResponse(url="http://localhost:8002/items", status_code=302)

@app.get("/redirect-perm")
async def redirect_permanent():
    """Returns 301 permanent redirect"""
    return RedirectResponse(url="http://localhost:8002/items", status_code=301)

@app.get("/redirect-see-other")
async def redirect_see_other():
    """Returns 303 See Other redirect"""
    return RedirectResponse(url="http://localhost:8002", status_code=303)
